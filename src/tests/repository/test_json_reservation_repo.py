from src.repository.reservation_repository_json import ReservationRepositoryJSON
...
def temp_json_file(tmp_path):
    file_path = tmp_path / "reservations.json"
    return str(file_path)

@pytest.fixture
def repository(temp_json_file):
    return ReservationRepositoryJSON(temp_json_file)

def test_add_reservation(repository, temp_json_file):
    ...
    with open(temp_json_file, "r") as file:
        data = json.load(file)
    assert "1" in data
    assert data["1"]["guest_name"] == "John Doe"
...
def test_remove_reservation(repository, temp_json_file):
    reservation = Reservation(
        room_number="103",
...
def test_update_reservation(repository, temp_json_file):
    reservation = Reservation(
        room_number="104",
    ...
    with open(temp_json_file, "r") as file:
        data = json.load(file)
    assert data["4"]["room_number"] == "105"
    assert data["4"]["departure_date"] == "2023-12-15"